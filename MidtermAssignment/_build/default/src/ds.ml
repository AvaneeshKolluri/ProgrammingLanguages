...
let rec let_rec_helper lst id : (Ast.decs option) =
  match lst with
  | [] -> None
  | (id', parameter, body)::t -> if id' = id then Some (* (Ast.decs *)[(id',parameter,body)]
                                             else let_rec_helper t id


let extend_env_rec_m decs : env ea_result =
  fun env  ->
...
let rec apply_env : string -> exp_val ea_result =
  ...
  | ExtendEnvRec(decs,tail) ->
    match (let_rec_helper decs id) with
    | None -> apply_env id tail
    | Some (* Ast.decs *)[(i,p,b)] -> Ok (ProcVal (p,b,env))
